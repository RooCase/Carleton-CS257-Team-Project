...
class InteractDataSource:
    ...
    def filter_schools(self):
        ...
        cursor = self.connection.cursor()
        cursor.execute(f"SELECT school_name FROM schools WHERE charter IN ({charter}) AND {grade} = any("
                       f"available_grades) AND enrollment BETWEEN {enrollment} ORDER BY school_name ASC")
        records = cursor.fetchall()
        print(records[0])
        return records
